import sys
sys.path.append("D:/BUCT-Teaching-Aids-main/BUCT-Teaching-Aids-main/API/python_backend/head_up_detection")

import db_utils
import detection_core

def test_full_flow():
    """测试完整流程：创建表→摄像头检测→存储→查询"""
    # 1. 首次运行：创建数据表（仅需执行一次）
    ...
    test_data = {
        ...
        "course_id": "math_101",
        "course_name": "高等数学（上）",
        "data_type": "video_frame",  # 改为视频帧类型
        "detection_device": "电脑摄像头-0",
        "remarks": "摄像头实时检测"
    }

    # 3. 调用摄像头实时检测（核心修改）
    try:
        head_up_rate = detection_core.detect_head_up_rate_from_camera(show_window=True, camera_index=0)
        print(f"摄像头检测平均抬头率：{head_up_rate}%")
    except Exception as e:
        # 检测失败时用模拟值
        head_up_rate = 85.2
        print(f"摄像头检测失败，使用模拟值：{head_up_rate}%，原因：{str(e)}")
